...
async def check_permission(ctx, level):
    ...
    if now_user.permission >= level:
        ...
        embed.set_footer(text = f"Sented by {config.bot_name}ㆍUser Permission Error")
        if now_user.permission == 0 and level == 1:
            embed.add_field(name = "Suggestion", value = f"/accept_term으로 약관 동의를 하시면, `{permission_message[1]}` 권한을 얻어, 이 명령어를 실행 하실 수 있습니다.", inline = False)
