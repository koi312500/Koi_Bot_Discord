...
class Development(commands.Cog):
    ...
    async def update_command(self, ctx):
        ...
        command_data = command_data + fd_popen.read().strip()
        fd_popen.close()
        embed = discord.Embed(title = '/test Koi_Bot Slash Result', description = 'git pull command', color = 0x00ffff)
        embed.add_field(name = "Result of the 'git pull origin master' & 'pip install -r requirements.txt'", value = command_data, inline = False)
        await ctx.respond(embed = embed)
