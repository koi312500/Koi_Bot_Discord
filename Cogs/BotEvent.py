    ...
    async def on_application_command_error(self, ctx, error):
        ...
        try:
            if isinstance(error.original, discord.Forbidden):
                error_notfound = False
                embed = discord.Embed(title = f"이 명령어를 {config.bot_name} 의 권한 부족으로 실행하지 못했습니다.", description = "관리자에게 권한 추가를 요청해 보세요.", color = 0xff0000)
        except:
            pass
    
        if error_notfound == True:
            embed = discord.Embed(title="Error Info", description=f"{config.bot_name} Error Info", color=0xff0000)
